...
public enum CouponCommon {
	OK(200, "SUCCESS", 200)
	, INVALID_PARAMETER(4000, "Invalid parameter.", HttpStatus.BAD_REQUEST.value())
	, NOT_ALLOWED(4050, "Not allowed", HttpStatus.METHOD_NOT_ALLOWED.value())

	, AUTH_NOT_LOGGED_IN(21001, "로그인이 필요합니다.", 404)
	, AUTH_NOT_EXISTS_PID(21002, "회원번호 확인 후 다시 시도해주세요.", 400)
	, AUTH_NOT_EXISTS_USER(21002, "회원번호 확인 후 다시 시도해주세요.", 400)
	...
	CouponCommon(final int code, final String msg, final int status) {
		this.errorCode = code;
